...
class TrilaterationView extends StatefulWidget {
  ...
  @override
  State<TrilaterationView> createState() =>
      TrilaterationViewState();
}
                ...
                SizedBox(
                  child: MapGridView(
                    ...
                    radiusList: controller.selectedDevices
                        .map((device) => logDistancePathLoss(device.rssi.toString(), device.alpha))
                        .toList(),
                  ),
                ),
                    ...
                    if (snapshot.hasData) {
                      ...
                      return ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          String rssi = data.rssi.toString();
                          double alpha = selectedDevice.alpha;
                          num distance = logDistancePathLoss(rssi, alpha);
                          // selectedDevice.distance = distance;
                          //num distance = selectedDevice.distance;
                                  ...
                                  children: <Widget>[
                                    Padding(
                                      ...
                                      child: SpinBox(
                                        min: -100,
                                        max: -30,
                                        value: selectedDevice.alpha,
                                        decimals: 0,
                                        step: 1,
                                        onChanged: (value) {
                                          setState(() {
                                            selectedDevice.alpha = value;
